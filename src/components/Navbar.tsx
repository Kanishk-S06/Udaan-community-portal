...
const Navbar = (props: searchProp) => {
  ...
  return (
    <div className="flex pl-20 pt-3 shadow-md h-14 w-screen">
      <img src={udaan} className="w-24 h-7" />
      ...
      <div className="flex border border-spacing-1 h-9 ml-10 w-72 p-1 cursor-pointer">
        ...
        <input onChange={(e) => props?.setSearch(e.target.value)} placeholder="Search UDAAN" className="ml-2 outline-none" />
      </div>
      ...
      <h1 onClick={() => setPost(true)} className="bg-blue-700 rounded-full text-sm cursor-pointer text-white w-36 pl-6 ml-6 pt-2 h-9">Add question</h1>
